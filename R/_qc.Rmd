...
```{r multiqc-import}
multiqc_files <- list.files('multiqc_data', full.names = T, pattern = '*.txt')

multiqc <- purrr::map(multiqc_files, ~ read.delim(.x, header = T))
multiqc <- lapply(multiqc_files, read.delim) 
names(multiqc) <- sapply(stringr::str_split(multiqc_files,'/|_|\\.'), function(x) paste(x[1],x[4],sep ='.'))
