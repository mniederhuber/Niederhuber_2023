...
dep %>% dplyr::filter(dep$Package == 'profileplyr')
```
...
``` {r}
replicate_sheet <- read.csv('../rnaiScreen_replicate_wings.csv') %>%
...
```
file 'quant.csv' is a manual measurement done in FIJI
...
dlD.ratio %>%
  ggplot(aes(rnai, ratio)) + 
  geom_boxplot(aes(fill = rnai)) +
  geom_point(position = position_jitter(width = 0.08)) +
  scale_fill_manual(values = c('grey60','#CC0066'), name = 'RNAi') +
  #annotate(geom = 'segment', x = 1, xend = 2, y = 1.5, yend = 1.5) +
  #annotate(geom = 'text', x = 1.5, y = 1.51, label = '***') +
  ...
  theme(axis.title.x = element_blank(),
        ...
        axis.text.y = element_text(size = 12),
        panel.background = element_rect(fill = "#d5e1e3"))
ggsave('rPlots/dlD.png', width = 3, height = 4, dpi = 300)
