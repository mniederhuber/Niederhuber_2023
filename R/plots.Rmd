...
```
## E - Diff Bind
osaDeg.qF -- 6877 peaks

This peak list is slightly more than the peaks$faire.osaDeg.df
These are ranges from diffbind, which i need for the log2 and pvals. 
I don't currently have a clean way to annotate the main df with those values.
...
hairless.df %>%
  ...
  geom_bar(stat = 'identity') + 
  geom_text(aes(label = n, y = frac), vjust = 3,  size = 6, position = 'stack') +
  scale_fill_manual(values = c('#2ad4c3','grey60'), name = 'Overlaps\nOsa Peak') +
  annotate('text', x = 1, y = 1.02, label = '***') +
  theme(axis.title = element_blank(),
        axis.text = element_text(size = 12), 
        axis.ticks.x = element_blank())
ggsave('rPlots/6E-hairless_overlap.png', dpi = 300, width = 3, height = 4)
...
## D - Dl upregulation quantification
```{r}
