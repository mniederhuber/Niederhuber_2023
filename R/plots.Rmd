...
png('rPlots/S3-clusters.png', width = 3, height = 4, res = 300, units = 'in')
...
#  scale_color_manual(values = c('#DE4A47', '#9CBD36')) +
  ...
  theme(legend.position = 'none')
ggsave('rPlots/3G-1.png', width = 3, height = 5)
...
ggsave('rPlots/3G-2.png', width = 2, height = 6)
```
...
enrichtest.hairless <- ChIPseeker::enrichPeakOverlap('bed/osaPeaks-enhancers-full.bed', 
...
deg.hms <- purrr::map(names(deg.mats), function(x) {
  if(grepl('control.Rep1',x)){
     EnrichedHeatmap(deg.mats[[x]],
                    ...
                    name = 'zScore',
                    row_title = NULL,
                    top_annotation = HeatmapAnnotation(enriched = anno_enriched(gp = gpar(col = cols, lwd = 2), axis_param = list(side = 'left', facing = 'outside'), ylim = c(0,6))),
...
png('rPlots/4B.png', width = 3, height = 5, units = 'in', res = 300)
