...
ComplexHeatmap::Heatmap(tidymat, split = k8$cluster, cluster_columns = F, show_row_dend = F, col = figure_col_fun, gap = unit(0.5, "lines"),
                        show_row_names = F)
...
#  tidyr::unnest(cols = c(flank_gene, flank_distance)) 
    ...
    dplyr::select(Geneid,k8.cluster,c(72:83)) %>%
    tidyr::pivot_longer(c(3:14)) %>%
    dplyr::distinct() %>%
...
#                  stage = as.integer(stage),
 ...
 #   dplyr::summarise(mean = mean(value)) %>%
    ...
    dplyr::mutate(norm = rep_mean/rep_mean[stage == 'L3'],
    ...
    geom_ribbon(aes(y = mean, ymin = mean-(se), ymax = mean+(se)), alpha = 0.2) +
...
##                  stage = as.integer(stage),
 ...
 ##   dplyr::summarise(mean = mean(value)) %>%
 #   dplyr::group_by(Geneid, stage, k8.cluster) %>%
 #   dplyr::summarise(rep_mean = mean(value)) %>%
 #   dplyr::group_by(Geneid, k8.cluster ) %>%
 ...
 #                    sd = sd(rep_mean)) %>%
 #   ggplot(aes(stage, mean, group = k8.cluster, color = k8.cluster) ) +
 #   geom_ribbon(aes(y = mean, ymin = mean-(sd/2), ymax = mean+(sd/2))) +
 #   geom_line(size = 2, color = 'black') +
 #   theme(legend.position = 'none')
