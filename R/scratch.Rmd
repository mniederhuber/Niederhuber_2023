...
ol.shuffle <- dds.dep %>%
  data.frame() %>%
  dplyr::mutate(shuffle = ifelse(GRanges(.) %over% osa.enhancer.shuffle, T, F)) %>%
  dplyr::group_by(shuffle) %>%
  dplyr::tally() %>%
  dplyr::mutate(frac = n/sum(n),
                id = 'Shuffle')

ol.dep.n <- ol.dep[ol.dep$osa.enhancer,]$n
ol.shuffle.n <- ol.shuffle[ol.shuffle$shuffle,]$n
dep.sum <- sum(ol.dep$n)
shuffle.sum <- sum(ol.shuffle$n)
ol.shuffle.n

prop.test(c(ol.dep.n, ol.shuffle.n), c(dep.sum, shuffle.sum), correct = T)
```
...
tidy.dds <- dds.peaks.qF %>% 
  dplyr::filter(log2FoldChange <= -1) %>%
     dplyr::select(peak, group, dplyr::matches("faire.\\d{1,2}h.zScore|faire.3LW.zScore")) %>%
     tidyr::pivot_longer(dplyr::matches("faire.\\d{1,2}h.zScore|faire.3LW.zScore")) %>%
     dplyr::mutate(stage = stringr::str_split_fixed(name,'\\.', n = 3)[,2],
                   stage = factor(stage, levels = c('3LW', '6h', '18h','24h','36h','44h'))) %>%
#                   k8.cluster = factor(k8.cluster, levels = c('2', '3', '6','1','5','8','7','4'))) %>%
