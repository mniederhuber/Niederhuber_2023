...
plot.df <- cluster.df %>%
    dplyr::distinct(peak, .keep_all = T) %>%
     ...
     dplyr::group_by(km.cluster, stage, WT.3LW) %>%
     dplyr::summarise(mean = mean(value),
                      sd = sd(frac) ,
                      n = dplyr::n()) %>%
  dplyr::group_by( WT.3LW, stage) %>%
...
ggsave('rPlots/3-draft_fraction_cluster.png', width = 4, height = 9)

## fraction barplot
plot.df %>% 
  dplyr::ungroup() %>%
  #dplyr::mutate(km.cluster = forcats::fct_rev(km.cluster)) %>%
  dplyr::select(WT.3LW, km.cluster, n, per) %>%
  dplyr::distinct() %>%
  dplyr::mutate(group = dplyr::case_when(km.cluster %in% c('3','6','5','7') ~ 'Closing',
                                         km.cluster %in% c('2','1','8') ~ 'Opening',
                                         km.cluster %in% c('4') ~ 'Static')) %>%
  ggplot(aes(per, km.cluster, fill = group)) +
  geom_bar(stat = 'identity') +
  #coord_polar(theta = 'y') +
  geom_text(aes(x = per, hjust = -0.1, label = paste0(round(per, digits = 1),'%')), size = 5) +
  scale_fill_manual(values = c('#ff2937','#85b524','#9c9c9c')) +
  ...
  strip.text.x = element_blank(),
  panel.spacing.y = unit(1, 'lines')) 
ggsave('rPlots/3-draft_fraction_cluster-pie.png', width = 4, height = 8)
```
...
ChIPpeakAnno::makeVennDiagram(list(rn.union, osaPeaks.full),
