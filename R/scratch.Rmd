...
distances %>%
 ...
 # geom_vline(xintercept = c(0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000), color = 'grey50', linetype = 'dashed') +
  geom_histogram(aes(y = after_stat(count), fill = meanZ, group = meanZ), size = 1, binwidth = 100) +
...
distances %>%
  ...
  dplyr::select(peak, frac_distance) %>%
  dplyr::group_by(frac_distance) %>%
  dplyr::tally() %>%
  dplyr::mutate(per = n/sum(n)) %>%
  ggplot(aes('', y = per, fill = frac_distance)) +
...
```{r}
# 200kb bins
bins <- GenomicRanges::tileGenome(GenomeInfoDb::seqinfo(dm6),tilewidth = 2e05) %>% 
  unlist() %>%
  data.frame() %>%
