...
ChIPpeakAnno::makeVennDiagram(list(peaks %>% dplyr::filter(assay == 'cnr' & yw.cnr) %>% GRanges(),
                                   peaks %>% dplyr::filter(assay == 'cnr' & osa.cnr) %>% GRanges()))

###

peaks %>%
  dplyr::filter(assay == 'cnr' & yw.cnr | assay == 'cnr' & osa.cnr) %>%
  dplyr::mutate(twofold = ifelse(cnr.log2FoldChange <= -1 | cnr.log2FoldChange >= 1, T, F),
                cat = dplyr::case_when(osa.cnr & !yw.cnr ~ 'Osa',
                                       !osa.cnr & yw.cnr ~ 'Control',
                                       T ~ 'Shared')) %>% 
  ggplot(aes(x = '',  cnr.log2FoldChange, fill = cat)) + 
...
Quantification of the precentage of A and T bases in 3kb windows around osa peaks. 
ie. a the fraction of peaks at each position of a 3kb window that have A or T bases.
Suggests there is lower A/T content within osa binding sites. 

Sanity check that summit coordinates don't change this result, and they don't. 
Summit coords are likely off center from the narrowpeak coordinate center, so maybe there's a difference in AT content
around summits.
But that doesn't appear to be true, still saw a drop in AT content. 
```{r}
...
--- old ----
1. Identify index for nearest osa peak for all dm6 genes 
2. join the dm6 gene ranges object with peak coordinates using osa peak index
3. calculate distance between gene start and osa peak center
4. select the top 3 closest genes for each osa peak
5. join with wing rna seq data and calculate log2fold change of expression between L3 and 24h
6. obtain list of gene id for all genes near osa peaks that overlap with faire peaks that close from l3 to 24h and have reduced expression in the same period

Logic -- sites with reduced accessibility may be more likely to have a regulatory connection to nearby genes with reduced expression

result -- there are 89 downreg genes near osa bound closing sites
  these genes are enriched for BP GO terms associated with:
    "cell fate specification",
    "Dendrite development," 
    "wing morphogenesis," and
    "transcriptional regulation - positive and negative"
   
   
** trying with control for all down regulated genes 
still get dendrite, but it drops some on the list
##### GO
```{r}
dm6.genes <- GenomicFeatures::genes(dm6.TxDb) 

faire.closing.osaBound <- peaks %>% 
  dplyr::filter(assay == 'faire'  & experiment == 'WT FAIRE Wing Timecourse' & faireCat.3LW_24h == 'closing' & osa.cnr & !yw.cnr) %>%
  GRanges()


nearest <- GenomicRanges::nearest(x = dm6.genes, subject = faire.closing.osaBound)
#nearest <- GenomicRanges::nearest(x = faire.closing.osaBound, subject = dm6.genes)

dm6.genes.df <- dm6.genes %>%
  data.frame() %>%
  dplyr::mutate(row = 1:nrow(.),
                osaPeak.index = nearest) %>%
  dplyr::filter(!is.na(osaPeak.index)) %>%
...
dm6.genes.osaPeaks.rna <- faire.closing.osaBound %>%
  data.frame() %>%
  dplyr::mutate(osaPeak.index = 1:nrow(.), .before = 1) %>%
  dplyr::left_join(dm6.genes.df, ., by = 'osaPeak.index') %>%
  dplyr::mutate(distance = abs(start.x - start.y)) %>%
  dplyr::group_by(osaPeak.index) %>%
  dplyr::slice_min(distance, n = 2) 


ChIPseeker::seq2gene(faire.closing.osaBound, tssRegion = c(-1000,1000), flankDistance = 3000, dm6.TxDb)


#dm6.genes.osaPeaks.rna %>%
#  ggplot(aes(distance)) +
#  geom_histogram() +
#  xlim(c(0,100000))
#
#faire.closing.osaBound %>%
#  data.frame() %>%
#  ggplot(aes(abs(distanceToTSS))) + 
#  geom_histogram() + 
#  xlim(c(0,100000))


#redundant filter for closing here...
closing.down.osaBound <- dm6.genes.osaPeaks.rna %>%
  dplyr::filter(L3_mean >= 2 & faireCat.3LW_24h == 'closing' & cat == 'down') %>%
  dplyr::select(Geneid) 

#write.csv(closing.down.osaBound$Geneid, file = 'closing-downReg-osaBound-GeneID.csv', sep = ',')

down.dm6 <- dm6.genes.df %>%
  dplyr::filter(L3_mean >= 2 & cat == 'down') %>%
  dplyr::select(Geneid) 

control <- select(org.Dm.eg.db, 
       keys = down.dm6$Geneid, 
       columns = c('SYMBOL','GO','ENTREZID'),
       keytype = "FLYBASE") 

closing.down.osa.go <- select(org.Dm.eg.db, 
       keys = closing.down.osaBound$Geneid, 
       columns = c('SYMBOL','GO','ENTREZID'),
       keytype = "FLYBASE")


#terms <- select(GO.db, 
#       keys = closing.down.osa.go$GO, 
#       columns = c('DEFINITION', 'GOID', 'TERM'))
#
#dplyr::bind_cols(closing.down.osa.go, terms)
#library(GOSemSim)

#dmGO <- GOSemSim::godata('org.Dm.eg.db', ont = 'MF')
#dependent.entrez
entrez <- closing.down.osa.go %>% .$ENTREZID
control.entrez <- control %>% .$ENTREZID

ggo <- clusterProfiler::enrichGO(gene = entrez,
                                 universe = control.entrez,
                                 OrgDb = org.Dm.eg.db,
                                 ont = 'ALL',
                                 readable = T)
ggo %>% data.frame()
...
##### nearest genes to osa bound closing by expression changes
```{r}
dm6.genes.osaPeaks.rna %>%
  dplyr::select(Geneid, faireCat.3LW_24h, cat, peak, L3_mean, h24_mean) %>%
  dplyr::filter(L3_mean >= 2 | h24_mean >= 2) %>%
  tidyr::pivot_longer(c(L3_mean, h24_mean)) %>%
  dplyr::group_by(name) %>%
  dplyr::mutate(stage = stringr::str_remove(name, 'h'),
                stage = stringr::str_split_fixed(stage, '_', n = 2)[[1]],
                stage = factor(stage, levels = c('L3', '24')),
                Geneid = factor(Geneid),
                Geneid = forcats::fct_reorder(Geneid, cat)) %>%
  dplyr::group_by(Geneid) %>%
  dplyr::mutate(fmax = value/max(value)) %>%
  dplyr::group_by(faireCat.3LW_24h, cat) %>%
  dplyr::tally() %>%
  dplyr::mutate(frac = n/sum(n)) %>%
  dplyr::filter(faireCat.3LW_24h != 'NA') %>%
#  dplyr::filter(faireCat.3LW_24h == 'closing' & cat == 'down') %>%
  ggplot(aes(faireCat.3LW_24h, frac, fill = cat, label = n)) +
  geom_bar(stat = 'identity') +
  geom_text(position = position_stack(vjust = 0.5), size = 8) +
  scale_fill_manual(values = c('#e8335d','grey80','#5ae342'), name = 'Expression\n3LW-24h') +
  theme(axis.title = element_blank(),
        axis.text = element_text(size = 12))
ggsave('rPlots/3-draft-osabound-closing-nearestGeneexpression.png', width = 4, height = 3, dpi = 300)
```


#### are there hubs of differentially regulated genes around osa bound sites?
ie. do osa bound closing sites tend to be found around clusters of downregulated genes?

answer - no doesn't seem to be the case
```{r}
dm6.genes.osaPeaks.rna %>%
  dplyr::filter(faireCat.3LW_24h != 'NA') %>% 
  dplyr::filter(L3_mean >= 2 | h24_mean >= 2) %>%
  dplyr::group_by(osaPeak.index, faireCat.3LW_24h, cat) %>%
  dplyr::tally() %>%
  ggplot(aes(faireCat.3LW_24h, n, color = cat)) +
  geom_violin()
```
...
dm6.genes.osaPeaks.rna %>%
  dplyr::filter(faireCat.3LW_24h != 'NA') %>%
  dplyr::filter(L3_mean >= 2 | h24_mean >= 2) %>%
 # dplyr::filter(faireCat.3LW_24h %in% c('closing','static')) %>%
  dplyr::mutate(distance = abs(start.y - start.x)) %>%
  ggplot(aes('', l3v24, fill = cat)) +
...
```{r}
wing.rnaseq %>%
  dplyr::filter(Geneid == 'FBgn0263930') %>%
  dplyr::select(!mean, !log_mean) %>%
  tidyr::pivot_longer(!Geneid) %>%
  dplyr::group_by(name) %>%
  dplyr::mutate(name = stringr::str_remove(name, 'X'), 
                stage = stringr::str_split_fixed(name, '_',n = 2)[[1]],
...
### e93 sensitive sites
doesn't look like there is a correlation between osa binding and sites that are sensitive to e93 in discs
using liftover to convert dm3 coordinates from geo data, so may be worth verifying with my own data...
...
most Osa binding sites are static from 3LW to 24h, only ~30% of osa sites close. 
```{r}
osaPeaks %>%
  data.frame() %>%
  dplyr::group_by(faireCat.3LW_24h) %>%
...
Are the osa bound sites that close late near genes that are temporally dynamic and in what direction?

Most of the genes near osa binding sites that close are static between 3LW and 24h, 
however these closing sites are more frequently near genes that go down in expression than sites that open or are static.
```{r}
...
brFimo <- memes::runFimo(brD.seq, universalmotif::to_list(meme_db_on))
brFimo %>% 
  data.frame() %>% 
...
#streme.bySummit <- summit.seq.byRep %>%
#  memes::runStreme(control = 'shuffle')

#memes::runTomTom(streme.bySummit)

#streme.bySummit

#ame
#ame.bySummit <- osaBound.seq %>%
#  memes::runAme()
#ame.bySummit %>%
...
#  setNames(osaBound.streme$name)
#names(osaBound.streme$motif) <- osaBound.streme$name

purrr::map(names(osaBound.streme$motif), function(x) {
  fp = paste0('rPlots/motif-analysis/osaBound_sreme-',x,'.png')
  #return(osaBound.streme$motif[[.]])
  universalmotif::view_motifs(osaBound.streme$motif[[x]] ) %>%
    ggsave(fp, plot = ., device = 'png', width = 4, heigh = 3, dpi = 300)
  })


#motif_plots <- purrr::map(names(osaBound.streme$motif), ~ {
#  universalmotif::view_motifs(osaBound.streme$motif[[.]])
#  }) %>%
#  setNames(names(osaBound.streme$motif))
#
#names(motif_plots) <- names(osaBound.streme$motif)
#
#
#
#plotnames = purrr::imap(motif_plots, ~paste0('rPlots/motif-analysis/',.y,"_",names(.x),".png")) %>%
#  purrr::flatten()
#
#  fp = paste0('rPlots/motif-analysis/osaBound_streme-',.,'.png')
#
#
#
#osaBound.streme %>%
#  purrr::map(.$motif, ~ return(.)) 
#  setNames(paste0(.$name,'.png')) %>%
#  purrr::map(png::writePNG())
  
osaBound.streme %>%
  dplyr::mutate(consensus = forcats::fct_reorder(name, log_evalue),
                log_evalue = as.double(log_evalue)) %>%
  dplyr::slice_min(order_by = log_evalue, n = 5) %>%
  ggplot(aes('', y = name, fill = nsites)) + 
  geom_raster()
  geom_point(size = 5)
  
  
memes::runTomTom(osaBound.streme,database = '/Users/m/McKay/Motif_databases/FLY/fly_factor_survey.meme' )
```
```{r}
osaBound.dreme %>%
