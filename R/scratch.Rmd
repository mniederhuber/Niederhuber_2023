...
Gviz::plotTracks(c(axis, cnr.tracks, peakTrack, brdisc.at), chromosome = seqnames(brD) %>% as.character(), 
                 from = start(brD + 1000),
                 to = end(brD + 1000),
                 cex.axis = 0.3,
                 cex.title = 0.5,
...
Gviz::plotTracks(c(axis, cnr.tracks, peakTrack, genetrack), chromosome = seqnames(espl) %>% as.character(), 
...
Gviz::plotTracks(c(axis, cnr.tracks, peakTrack, genetrack), chromosome = seqnames(Dl) %>% as.character(), 
                 from = start(Dl),
                 to = end(Dl),
                 cex.axis = 0.3,
                 cex.title = 0.5,
...
Gviz::plotTracks(c(axis, cnr.tracks, peakTrack, genetrack), chromosome = seqnames(sbb) %>% as.character(), 
...
multiBamSummary bins --bamfiles Bam/*deGrad*dupsRemoved.bam \
  -o R/rPlots/multiBamsum.npz

plotCorrelation -in R/rPlots/multiBamsum.npz \
  -c spearman -p heatmap --colorMap RdYlBu --plotNumbers \
  --labels control.1 deGrad.1 control.2 deGrad.2  \
  --removeOutliers --skipZeros \
  -o R/rPlots/multiBamsum-spearman.png
  
plotPCA -in R/rPlots/multiBWsum.npz \
  --labels control.1 deGrad.1 control.2 deGrad.2  \
  -o R/rPlots/multiBWsum-PCA.png
```
## multiBWsum
```{bash engine.opts='-l'}
multiBigwigSummary bins -b R/BigWig/*deGrad*dupsRemoved_rpgcNorm_zNorm.bw -o R/rPlots/multiBWsum.npz

plotCorrelation -in R/rPlots/multiBWsum.npz \
  --corMethod spearman --skipZeros \
  --labels control.1 deGrad.1 control.2 deGrad.2  \
  --whatToPlot heatmap --colorMap RdYlBu --plotNumbers \
  -o R/rPlots/multiBWsum-spearman.png
