...
osaPeaks %>%
  ...
  ggplot(aes(faire.3LW.zScore, neglogp, color = cnr.peakCat)) +
  geom_point()
```
...
cluster.df <- data.frame(k8$cluster) %>%
  ...
  dplyr::left_join(fairePeaks.anno, ., by = 'peak') %>%
  dplyr::filter(anno.new %in% c('Distal Intergenic', 'Intron')) %>%
...
purrr::map(unique(cluster.df$k8.cluster), function(x) {
 ...
 piechart <- cluster.df %>%
    ...
    coord_polar(theta = 'y') + 
    scale_fill_manual(values = c('grey80', 'green')) + 
 ...
 rna <-  cluster.df %>%
   dplyr::filter(k8.cluster == x ) %>% 
    dplyr::select(Geneid,k8.cluster,c(70:81)) %>%
    ...
    dplyr::group_by(Geneid, stage, k8.cluster) %>%
   ...
   #dplyr::mutate(frac = rep_mean / max(rep_mean)) %>% # fraction of max per gene
    dplyr::group_by(k8.cluster,stage) %>% 
   ...
   dplyr::mutate(frac = mean/max(mean)) %>%
    ggplot(aes(stage, frac, group = k8.cluster,color = k8.cluster) ) +
...
cluster.df
 cluster.df %>%
   #dplyr::filter(k8.cluster == x ) %>% 
#  tidyr::unnest(cols = c(flank_gene, flank_distance)) 
    #dplyr::left_join(., wing.rnaseq, by = 'Geneid') %>%
    dplyr::select(Geneid,k8.cluster,c(70:81)) %>%
    tidyr::pivot_longer(c(4:15)) %>%
    dplyr::distinct() %>% #should drop any genes represented >1 per cluster
    dplyr::mutate(stage = stringr::str_split_fixed(name, '_', n = 2)[,1],
                 ...
                 # stage = stringr::str_remove(stage, 'h'),
                  stage = factor(stage, levels = c('L3', '6h', '18h','24h','36h','44h')),
#                  stage = ifelse(stage == 'L3', -6, stage),
#                  stage = as.integer(stage),
                  rep = stringr::str_split_fixed(name, '_', n = 2)[,2]) %>%
 #   dplyr::group_by(Geneid, stage, k8.cluster) %>%
 #   dplyr::summarise(mean = mean(value)) %>%
    dplyr::group_by(Geneid, stage, k8.cluster) %>%
    dplyr::summarise(rep_mean = mean(value)+0.001) %>%
    dplyr::group_by(Geneid, k8.cluster) %>%
   dplyr::filter(any(rep_mean >= 2)) %>%
    dplyr::mutate(norm = log2(rep_mean/rep_mean[stage == 'L3'])) %>%
#   dplyr::filter(!is.na(norm)) %>%
#   dplyr::filter(norm != Inf | norm != -Inf)  %>%
                  #norm = ifelse(is.na(norm) | norm == Inf, 0, norm)) %>%
    dplyr::group_by(k8.cluster,stage) %>%
    dplyr::summarise(mean = mean(norm), 
                     sd = sd(norm),
                     n = dplyr::n(),
                     se = sd / sqrt(n)) %>%
    ggplot(aes(stage, mean, group = k8.cluster, color = k8.cluster) ) +
   # geom_ribbon(aes(y = mean, ymin = mean-(sd/2), ymax = mean+(sd/2)), alpha = 0.2) +
    geom_line(size = 2, color = 'black') +
    theme(legend.position = 'none') +
    facet_wrap(~k8.cluster, scales = 'free_y')
  cluster.df %>%
    dplyr::distinct(peak, .keep_all = T) %>%
    dplyr::group_by(osabound) 
    dplyr::tally()  
  ...
  # dplyr::filter(k8.cluster == x ) %>% 
   ...
   dplyr::mutate(frac = rep_mean / max(rep_mean)) %>% # fraction of max per gene
    dplyr::group_by(k8.cluster,stage) %>% 
    dplyr::summarise(mean = mean(frac), 
                     sd = sd(frac)) %>% #average all the fraction of max together per stage per cluster
#   dplyr::group_by(k8.cluster) %>%
#   dplyr::mutate(frac = mean/max(mean)) %>%
    ggplot(aes(stage, mean, group = k8.cluster,color = k8.cluster) ) +
    geom_line(size = 2, color = 'black') +
