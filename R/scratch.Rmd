...
doesn't look like there is a greater occurence of down regulated genes around/near closing osa bound category of faire peaks
```{r}

fairePeaks.dm6 %>%
  dplyr::filter(osa.cnr & !yw.cnr & faireCat.3LW_24h != 'opening') %>%
  dplyr::select(peak, faireCat.3LW_24h, cat) %>%
  #dplyr::distinct()%>%
  dplyr::group_by(peak, faireCat.3LW_24h, cat) %>%
  dplyr::tally() %>%
  dplyr::group_by(faireCat.3LW_24h, cat, n) %>%
  dplyr::tally() %>%
#  dplyr::filter(faireCat.3LW_24h == 'closing') %>%
  ggplot(aes(n, nn, fill = cat)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  facet_grid(cat~faireCat.3LW_24h)
...
ie. do osa bound closing sites tend to be found around clusters of
downregulated genes?
