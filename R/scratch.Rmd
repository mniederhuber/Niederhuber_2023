...
purrr::map(unique(cluster.df$k8.cluster), function(x) {
 ...
 piechart <- cluster.df %>%
    ...
    coord_polar(theta = 'y') + 
    scale_fill_manual(values = c('grey80', 'magenta')) + 
...
test <- GOSemSim::mgeneSim(cluster$Geneid, semData = goData, verbose = F)

test2 <- test %>% data.frame() %>% tibble::rownames_to_column() %>% tidyr::pivot_longer(!rowname, names_to = 'colname')



test2.symbol <- AnnotationDbi::select(org.Dm.eg.db, keys = test2$rowname, columns = 'SYMBOL', keytype = 'FLYBASE') %>% dplyr::distinct()

out <- test2 %>%
  dplyr::rowwise() %>%
  dplyr::mutate(rowSymbol = test2.symbol[rowname == test2.symbol$FLYBASE,]$SYMBOL,
                colSymbol = test2.symbol[colname == test2.symbol$FLYBASE,]$SYMBOL)
