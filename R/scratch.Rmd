...
png('rPlots/3B-sbb.png', width = 7, height = 4, res = 300, units = 'in')
...
#png('rPlots/3D-all.png', width = 5, heigh = 6, units = 'in', res = 300)
...
#png('rPlots/osaPeaks-FAIREtimecourse.png', width = 5, height = 4, res = 300, units = 'in')
...
#  dplyr::mutate(k8.cluster = factor(k8.cluster, levels = cluster.levels)) 
...
```
##### testing matrix of T F peak timecourse
25% (119) of osa bound closed sites are called a peak later at some point in the FAIRE timecourse
...
osaPeaks.enhancers <- osaPeaks %>%
  data.frame() %>%
  dplyr::filter(anno.new %in% c('Promoter')) %>%
  GRanges()
...
png('rPlots/3D-promoters.png', width = 5, heigh = 6, units = 'in', res = 300)
ComplexHeatmap::draw(fig3d.hms, row_split = groups, annotation_legend_list = list(lgd), row_title = NULL, heatmap_legend_side = "bottom", merge_legend = T)
dev.off()
...
## Drafts
### intersect with hairless dm6 chip-chip
```{r}
hairless <- read.table('hairless-dm6.bed', sep = '\t') 
...
hairless %>%
#  data.frame() %>%
  dplyr::mutate(osa = ifelse(GRanges(.) %over% osaPeaks, T, F)) %>%
  dplyr::group_by(osa) %>%
  dplyr::tally() %>%
  dplyr::mutate(frac = n/sum(n))
```

### intersect with smrter chip-chip
```{r}
#### smrter
smrter <- read.table('smrter-dm6.bed', sep = '\t') 

colnames(smrter) <- c('seqnames', 'start', 'end')
...
### intersect with ctbp peaks
```{r}
ctbp.ss <- read.csv('~/McKay/CtBP_CnR/sampleSheet.tsv', sep = '\t')


ctbp.peak <- getPeakData(ctbp.ss, narrowPeak_colname = 'peakFile', by = 'fraction') %>%  dplyr::filter(fraction == 'sup')

q <- quantile(ctbp.peak$qValue, probs = 0.5) #find the 75% qVal


ctbp.peak.qf <- ctbp.peak %>%
  dplyr::filter(qValue >= q)


ctbp.peak.qf %>%
  data.frame() %>%
  dplyr::mutate(osa = dplyr::if_else(GRanges(.) %over% osaPeaks, T, F)) %>%
  dplyr::group_by(osa) %>%
  dplyr::tally() %>%
  dplyr::mutate(frac = n/sum(n))
...
osaPeaks %>%
  data.frame() %>%
  dplyr::mutate(ctbp = dplyr::if_else(GRanges(.) %over% GRanges(ctbp.peak.qf), T, F)) %>%
  dplyr::group_by(ctbp) %>%
  dplyr::tally() %>%
  dplyr::mutate(frac = n/sum(n))
```
### intersect with ecr peaks
```{r}
ecr.ss <- read.csv('~/McKay/ecrGFSTF-3LW-Wing-CnR/sampleSheet.tsv', sep = '\t') %>%
  dplyr::mutate(peakFile = paste0('/Users/m/McKay/ecrGFSTF-3LW-Wing-CnR/',peak))

ecr.peak <- getPeakData(ecr.ss, narrowPeak_colname = 'peakFile', by = 'fraction') %>%
  dplyr::filter(fraction == 'sup')

#q <- quantile(ecr.peak$qValue, probs = 0.75) #find the 75% qVal
ecr.peak

#ecr.peak.qf <- ecr.peak %>%
#  dplyr::filter(qValue >= q)
ecr.peak
ecr.peak %>%
  #data.frame() %>%
  dplyr::mutate(ecr = dplyr::if_else(GRanges(.) %over% osaPeaks, T, F)) %>%
  dplyr::group_by(ecr) %>%
  dplyr::tally() %>%
  dplyr::mutate(frac = n/sum(n))
```
### intersect with rotund peaks
~20% of osa peaks overlap with q filtered and reproducible rn peaks
~20% of rotund peaks overlap with osaPeaks 
```{r}
...
ChIPpeakAnno::makeVennDiagram(list(rn.union, osaPeaks.enhancers),
                              NameOfPeaks = c('Rotund','Osa'))

rn.ss
rn.bws <- get_bws(rn.ss, '') 
rn.bws
```
...
distances <- GenomicRanges::distanceToNearest(osaPeaks.full) %>%
  data.frame() %>%
  dplyr::bind_cols(data.frame(osaPeaks.full), .) %>%
  dplyr::rowwise() %>%
  dplyr::mutate(meanZ = mean(c(osaGFP.rep1.zScore, osaGFP.rep2.zScore))) 

distances %>%
  ggplot(aes(distance, '')) +
 # geom_vline(xintercept = c(0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000), color = 'grey50', linetype = 'dashed') +
  geom_histogram(aes(y = after_stat(count), ), size = 1, binwidth = 100) +
#  scale_fill_viridis_c(name = 'Density')+
  xlab('Distance (kb)') +
  ylab('Count') +
  scale_fill_viridis_c() + 
  scale_x_continuous(limits = c(-10,5000), breaks = c( 0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000), 
                     labels = scales::label_number(scale = 1e-3)) +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 12))

ggsave('rPlots/3-peak_to_peak_distance.png', width = 4, height = 3, dpi = 300)


distances %>%
  ggplot(aes(distance, meanZ)) +
  #geom_point() +
  xlim(c(0, 1e04)) +
  geom_hex(bins = 50)


#distances.frac <- distances %>%
#  #dplyr::mutate(frac_distance = ifelse(distance <= 5000, '<=5kb','>5kb')) %>%
#  dplyr::mutate(frac_distance = dplyr::case_when(distance <= 1000 ~ '<=1kb',
#                                                 distance <= 5000 & distance > 1000 ~ '1-5kb',
#                                                 distance <= 10000 & distance > 5000 ~ '5-10kb',
#                                                 T ~ '>10kb')) 
#
#distances.frac %>%
#  dplyr::select(peak, meanZ,frac_distance) %>%
#  dplyr::group_by(frac_distance) %>%
#  dplyr::summarise(n = dplyr::n(),
#                   mean = mean(meanZ)) %>%
#  dplyr::mutate(per = n/sum(n)) 
#  ggplot(aes('', y = per, fill = mean, color = frac_distance)) +
#  geom_bar(stat = 'identity') +
# # coord_polar(theta = 'y') + 
#  geom_text(aes(label = n), position = position_stack(), color = 'black') +
#  scale_fill_viridis_c() +
...
```
#### kmean clustering distances
```{r}
dist.mat <- distances %>%
  dplyr::select(peak,distance) %>%
  tibble::column_to_rownames('peak') %>%
  as.matrix() 


set.seed(1230412)
k <- kmeans(dist.mat, 20, nstart = 24, iter.max = 15)

k.df <- k$cluster %>% data.frame('cluster' = .) %>%
  tibble::rowid_to_column('peak') 

distances %>% 
  dplyr::left_join(., k.df) %>%
  dplyr::select(meanZ, distance, cluster) %>%
  dplyr::mutate(cluster = as.factor(cluster)) %>%
  dplyr::group_by(cluster) %>%
  dplyr::summarise(meanD = mean(distance),
                   meanZ = mean(meanZ),
                   n = dplyr::n()) 
  ggplot(aes('', cluster, fill = mean)) +
  geom_tile()
```

#### binned cluster mapping across genome
```{r}

bins <- GenomicRanges::tileGenome(GenomeInfoDb::seqinfo(dm6),tilewidth = 10e03) %>% 
  unlist() %>%
  data.frame() %>%
  dplyr::filter(seqnames %in% c('chrX','chr2L','chr2R', 'chr3L', 'chr3R'))  %>%
  GRanges()

counts <- GenomicRanges::countOverlaps(bins, osaPeaks.full) %>% data.frame('count' = .) %>%
  dplyr::bind_cols(data.frame(bins), .) %>%
  dplyr::mutate(range = 1:nrow(.)) 


counts
genes <- GenomicFeatures::genes(dm6.TxDb)

ov <- subsetByOverlaps(genes, bins) 

bins %>%
  GenomicRanges::countOverlaps(., osaPeaks.full) %>% data.frame('count' = .) %>%
  dplyr::bind_cols(data.frame(bins))
  mcols(.)$gene_id
...
counts %>%
  dplyr::mutate(chr = paste0('chr',stringr::str_extract(seqnames, '\\d'))) %>%
  ggplot(aes(start, '', fill = count)) +
  geom_tile(aes(width = width), color = NA) +
  scale_fill_viridis_c() +
  theme(axis.title = element_blank(),
        axis.text = element_text(size =8),
        axis.ticks.y = element_blank()) +
  facet_wrap(~seqnames, ncol = 1, scales = 'free_x')
ggsave('rPlots/3-peak-cluster-distribution.png', width = 6, height =6, dpi = 300)



```
#### distance vs zscore scatter
doesn't actually look like there's a strong correlation between high amplitude peaks and clustering
```{r}
distances %>%
  dplyr::mutate(logD = log10(distance)) %>%
  ggplot(aes(distance, meanZ)) +
  geom_point() +
...
cluster.df <- data.frame(k8$cluster) %>%
  ...
  dplyr::mutate(osabound = ifelse(osa.cnr & !yw.cnr, T, F),
                k8.cluster = factor(k8.cluster, levels = cluster.levels)) %>%
  dplyr::rename('Geneid' = 'geneId') %>%
  dplyr::left_join(., wing.rnaseq, by = 'Geneid')
...
Plot the percentage of osa Peaks that overlap FAIRE cluster peaks. 
Decided to focus on 3 clusters for comparison, the two that are high at 3LW and close, relative to the static set. 

I was concerned that the static set would include a not insignificant number of sites that are relatively closed throughout the timecourse, so I added an additional `WT.3LW == T` filter.
Without this, I think it's difficult to argue for a biological connection between osa binding the closing groups rather than just binding open sites. ie. we need to be sure that the "static" group are in fact open at L3 like the other two categories. 
...
purrr::map(cluster.sub, function(x) {
  name <- paste0("cluster-",x)

 piechart <- cluster.df %>%
    dplyr::filter(k8.cluster == x & WT.3LW) %>% # adding in 3LW peak call filter -- logic is that I don't want to include static sites that are statically closed, or never really open. 
    dplyr::distinct(peak, .keep_all = T) %>%
   ...
   geom_text(aes(x = 1, label = n), position = position_stack(vjust = 0.5), color = c('black','#e667d3' )) +
  ...
  lineplot <- cluster.df %>%
    dplyr::filter(k8.cluster == x & WT.3LW) %>%
    dplyr::distinct(peak, .keep_all = T) %>%
     dplyr::select(peak, k8.cluster, dplyr::matches("faire.\\d{1,2}h.zScore|faire.3LW.zScore")) %>%
     tidyr::pivot_longer(dplyr::matches("faire.\\d{1,2}h.zScore|faire.3LW.zScore")) %>%
     dplyr::mutate(stage = stringr::str_split_fixed(name,'\\.', n = 3)[,2],
                   stage = factor(stage, levels = c('3LW', '6h', '18h','24h','36h','44h'))) %>%
#                   k8.cluster = factor(k8.cluster, levels = c('2', '3', '6','1','5','8','7','4'))) %>%
     dplyr::group_by(peak) %>%
     dplyr::mutate(frac = value/max(value)) %>%
 ...
 ggsave(plot = piechart, filename = paste0('rPlots/pieChart-',name,'.png'), width = 1.5, height = 1.5 , dpi = 300) 
...
```
##### dynamics of osa bound open vs closed
3lw open bound
```{r}
tidy.cluster <- cluster.df %>%
  dplyr::filter(osabound & WT.3LW) %>%
  dplyr::distinct(peak, .keep_all = T) %>%
     dplyr::select(peak, k8.cluster, dplyr::matches("faire.\\d{1,2}h.zScore|faire.3LW.zScore")) %>%
     tidyr::pivot_longer(dplyr::matches("faire.\\d{1,2}h.zScore|faire.3LW.zScore")) %>%
     dplyr::mutate(stage = stringr::str_split_fixed(name,'\\.', n = 3)[,2],
...
#                   k8.cluster = factor(k8.cluster, levels = c('2', '3', '6','1','5','8','7','4'))) %>%
     dplyr::group_by(peak) %>%
     dplyr::mutate(value = ifelse(value < 0, 0, value), # remove negative zScores
                   frac = value/max(value)) 
tidy.cluster %>%
  dplyr::group_by(k8.cluster) %>%
  tidyHeatmap::heatmap(peak, stage, value,
                       palette_value = circlize::colorRamp2(c(0, 1, 2, 3, 4, 5), viridisLite::viridis(6)),
                       cluster_rows = F, 
                       cluster_columns = F)
max(tidy.cluster$value)
```
...
```{r}
cluster.df %>%
  dplyr::filter(k8.cluster == 4 & WT.3LW) %>%
    dplyr::distinct(peak, .keep_all = T) %>%
  dplyr::group_by(osabound) %>%
  dplyr::tally() %>% 
  dplyr::mutate(frac = n / sum(n))
...
cluster <- data.frame(k8$cluster) %>%
  tibble::rownames_to_column('peak') %>%
  dplyr::mutate(peak = as.integer(peak)) %>%
  dplyr::left_join(peaks$faire.wt.df, ., by = 'peak') %>%
  dplyr::filter(assay == 'faire' & experiment == 'WT FAIRE Wing Timecourse') %>%
  dplyr::filter(anno.new %in% c('Distal Intergenic', 'Intron') & k8.cluster == 4 ) %>%
  dplyr::filter(osa.cnr & !yw.cnr) %>%
  memes::get_sequence(dm6)
...
ame <- memes::runAme(cluster, control = cluster.control) %>% 
...
closing.cluster <- cluster.df %>%
  dplyr::filter(WT.3LW & km.cluster %in% c('3','6','7','5') & !grepl('RNA',SYMBOL)) %>%
  dplyr::rename(Geneid = geneId) 

opening.cluster <- cluster.df %>%
  dplyr::filter(WT.3LW & km.cluster %in% c('2','1','8') & !grepl('RNA',SYMBOL)) %>%
  dplyr::rename(Geneid = geneId) 

opening.cluster <- cluster.df %>%
  dplyr::filter(WT.3LW & km.cluster %in% c('4') & !grepl('RNA',SYMBOL)) %>%
  dplyr::rename(Geneid = geneId) 

entrez <- clusterProfiler::bitr(closing.cluster$Geneid, 'FLYBASE','ENTREZID', OrgDb = org.Dm.eg.db)$ENTREZID
...
osaBound.ame %>%
  ...
  geom_text_repel(#data = dplyr::filter(osaBound.ame, adj.pvalue < 1e-7), 
    aes(label = symbol),
                  ...
                  box.padding = 0.7, 
                  max.overlaps = 20) 
#ggsave('rPlots/3-draft-motif-scatter.png', width =6, height = 5, dpi = 300)
...
pal = colorRampPalette(RColorBrewer::brewer.pal(n = 6, 'Spectral'))

ames <- purrr::map(c(T,F), function(x){
  name = ifelse(x,'open','closed')
  fp = paste0('rPlots/3-draft-ame-',name,'.png')
  p = paste0(name,'_adj.pvalue')
  
 query <- osaPeaks %>%
  data.frame() %>%
  dplyr::filter(anno.new %in% c('Distal Intergenic','Intron')) %>%
  dplyr::filter(WT.3LW == x) %>%
  GRanges() %>%
  resize(250, "center") %>%
  memes::get_sequence(dm6)
...
control <- fairePeaks.wt %>%
 ...
 ame <- query %>%
  ...
  dplyr::group_by(motif_alt_id) %>%
  dplyr::mutate(symbol = stringr::str_split_fixed(motif_alt_id, '_|-', n = 2)[[1]],
                neglog = -log(adj.pvalue)) %>%
   dplyr::rename(!!paste0(name,'_neglog') := neglog)
...
#  ggplot(aes('Osa Bound', symbol, fill = neglogP)) +
#  geom_raster() +
#  scale_fill_gradientn(limits = c(2,14),
...
#        axis.text.x = element_blank(),
#        axis.ticks.x = element_blank()) 
#ggsave(fp, width = 2.5, height = 2.5, dpi = 300) 
})


dplyr::left_join(ames[[1]],ames[[2]], by = 'symbol') %>%
  dplyr::select(symbol, dplyr::matches('*neglog')) %>%
  dplyr::mutate(group = dplyr::case_when(symbol %in% c('rn','sqz') ~ 'rn/sqz',
                                         symbol == 'pan' ~ 'pan',
                                         T ~ 'Other')) %>%
  ggplot(aes(open_neglog, 
             closed_neglog,
             fill = group)) +
  geom_point(size = 4,shape = 21) +
  ...
  geom_text_repel(aes(label = symbol),
                  ...
                  box.padding = 0.7, 
                  max.overlaps = 20) 
  ggsave('rPlots/3-draft-openvclosed_ame_scatter.png', width = 5, height = 4, dpi = 300)
...
sqz and rn have very similar / almost identicle motifs
```{r}
osa.enhancer.seq <- osaPeaks %>%
  data.frame() %>%
  dplyr::filter(anno.new %in% c('Distal Intergenic','Intron')) %>%
...
ggsave('rPlots/3E_osaBound-enhancer-3lwPeak.png', width = 2.5, height = 2.5, dpi = 300)
...
multiBamSummary bins --bamfiles Bam/*deGrad*dupsRemoved.bam \
  -o rPlots/multiBamsum.npz

plotCorrelation -in rPlots/multiBamsum.npz \
  -c spearman -p heatmap --colorMap RdYlBu --plotNumbers \
  --labels control.1 deGrad.1 control.2 deGrad.2  \
  --removeOutliers --skipZeros \
  -o rPlots/multiBamsum-spearman.png
  
plotPCA -in rPlots/multiBamsum.npz \
  --labels control.1 deGrad.1 control.2 deGrad.2  \
  -o rPlots/multiBamsum-PCA.png
```
## multiBWsum
```{bash engine.opts='-l'}
multiBigwigSummary bins -b BigWig/*deGrad*dupsRemoved_rpgcNorm_zNorm.bw -o R/rPlots/multiBWsum.npz
...
png('rPlots/4B-nogroups.png', width = 5, height = 4, units = 'in', res = 300)
...
#chr2R:7,158,233-7,182,249 prickle
...
pk <- data.frame(seqnames = 'chr2R',
  ...
  GRanges()
#arr
#chr2R:13,461,490-13,468,897
arr <- data.frame(seqnames = 'chr2R',
                    start = 13461490,
                    end = 13468897) %>%
  GRanges()

#ino80
#chr3R:19,346,618-19,374,766
ino <- data.frame(seqnames = 'chr3R',
                  start = 19364703,
                  end = 19374766) %>%
  GRanges()
#peakTrack <- Gviz::AnnotationTrack(range = osaPeaks.full, genome = 'dm6', fill = '#D01C8B', col = 'transparent', background.title = 'white', showTitle = F )
...
#eip78c
#chr3L:21,257,236-21,266,557
e78 <- data.frame(seqnames = 'chr3L',
                  start = 21257236,
                  end = 21266557) %>%
  GRanges()

#e93
#chr3R:21,977,280-21,981,625
e93 <- data.frame(seqnames = 'chr3R',
                  start = 21977280,
                  end = 21981625) %>%
  GRanges()
...
png('rPlots/4E-browser-prickle.png', width = 4, height = 4, res = 300, units = 'in')
Gviz::plotTracks(c(axis, deg.tracks, spacer, spacer, dds.track,spacer,spacer, genetrack),
...
png('rPlots/4E-browser-arr.png', width = 4, height = 4, res = 300, units = 'in')
Gviz::plotTracks(c(axis, deg.tracks, spacer, spacer, dds.track,spacer,spacer, genetrack), 
                 chromosome = seqnames(arr) %>% as.character(), 
                 from = start(arr),
                 to = end(arr),
                 cex.axis = 0.3,
                 cex.title = 0.5,
...
#ino80
png('rPlots/4E-browser-ino80.png', width = 4, height = 4, res = 300, units = 'in')
Gviz::plotTracks(c(axis, deg.tracks, spacer, spacer, dds.track,spacer,spacer, genetrack), 
                 chromosome = seqnames(ino) %>% as.character(), 
                 from = start(ino),
                 to = end(ino),
                 cex.axis = 0.3,
                 cex.title = 0.5,
...
#e74
png('rPlots/4E-browser-e74.png', width = 4, height = 4, res = 300, units = 'in')
Gviz::plotTracks(c(axis, deg.tracks, spacer, spacer, dds.track,spacer,spacer, genetrack), 
...
#e78
png('rPlots/4E-browser-e78.png', width = 4, height = 4, res = 300, units = 'in')
Gviz::plotTracks(c(axis, deg.tracks, spacer, spacer, dds.track,spacer,spacer, genetrack), 
                 chromosome = seqnames(e78) %>% as.character(), 
                 from = start(e78),
                 to = end(e78),
                 cex.axis = 0.3,
                 cex.title = 0.5,
...
#e93
png('rPlots/4E-browser-e93.png', width = 4, height = 4, res = 300, units = 'in')
Gviz::plotTracks(c(axis, deg.tracks, spacer, spacer, dds.track,spacer,spacer, genetrack), 
                 chromosome = seqnames(e93) %>% as.character(), 
                 from = start(e93),
                 to = end(e93),
                 cex.axis = 0.3,
                 cex.title = 0.5,
...
### Dl
 Gviz::plotTracks(c(axis, deg.tracks, spacer, spacer, dds.track,spacer,spacer, genetrack),
                  chromosome = seqnames(Dl) %>% as.character(), 
                 from = start(Dl-35000),
                 to = end(Dl-15000),
                 cex.axis = 0.3,
                 cex.title = 0.5,
...
test.db <- DiffBind::dba(sampleSheet = test)

test.db <- DiffBind::dba.count(test.db)
info <- DiffBind::dba.show(test.db)
libsizes <- cbind(LibReads=info$Reads,
                  FRiP=info$FRiP,
                  PeakReads=round(info$Reads * info$FRiP))
rownames(libsizes) <- info$ID 

test.db <- DiffBind::dba.normalize(test.db, normalize = 'lib') #library size normalization may be more appropriate for non-rnaseq data
norm <- DiffBind::dba.normalize(test.db, bRetrieve=T)
...
test.db <- DiffBind::dba.contrast(test.db, contrast = c('Condition', 'nubG4','control'))

test.db <- DiffBind::dba.analyze(test.db)

dds <- DiffBind::dba.analyze(test.db, bRetrieveAnalysis =TRUE) #retreive the deseq2 object

dds.results <- DESeq2::results(dds, contrast = c('Condition', 'nubG4', 'control'))

#DiffBind::dba.show(test.db, bContrasts = T, th = 0.1)


#dds.results %>% data.frame() %>%
#  ggplot(aes(log2FoldChange, padj)) +
#  geom_point(color = ifelse(dds.results$padj <= 0.1, 'red','black')) +
#  theme(axis.title = element_text(size = 12),
#        axis.text = element_text(size = 12))

#### 
test.db.peaks <- test.db$peaks %>% data.frame() %>%
  dplyr::select(seqnames, start, end) %>%
  dplyr::mutate(peak = 1:nrow(.)) 

dds.peaks <- dds.results %>% data.frame() %>%
  dplyr::mutate(peak = 1:nrow(.)) %>%
  dplyr::left_join(test.db.peaks, ., by = 'peak') 

dds.peaks.qF <- IRanges::subsetByOverlaps(GRanges(dds.peaks), fairePeaks.deg) %>%
  data.frame() %>%
...
dds.peaks.qF %>%
  data.frame() %>%
  ggplot(aes(log2FoldChange, padj)) +
  geom_point(color = ifelse(dds.peaks.qF$padj <= 0.1, 'red','black')) +
  theme(axis.title = element_text(size = 12),
        axis.text = element_text(size = 12))

ggsave('rPlots/4D-diffBind.png', width =4, height = 3.5, dpi = 300)  

dds.dep <- IRanges::subsetByOverlaps(GRanges(dds.peaks), fairePeaks.deg) %>%
  data.frame() %>%
  dplyr::filter(log2FoldChange <= -1) %>%
  GRanges() 
...
```
```{r}
osa.boot <- nullranges::bootRanges(osaPeaks.full, 
                                   blockLength = 500, 
                                   withinChrom = T,
                                   R = 50)
ol.dep <- dds.dep %>%
  data.frame() %>%
  dplyr::mutate(osa = ifelse(GRanges(.) %over% osaPeaks.full, T, F)) %>%
#                osa.enhancer = ifelse(GRanges(.) %over% osaPeaks.enhancers.full, T, F)) %>%
  dplyr::group_by(osa) %>%
  dplyr::tally() %>%
  dplyr::mutate(frac = n/sum(n),
                id = 'Osa Overlap')
...
ol.boot <- dds.dep %>%
  data.frame() %>%
  dplyr::mutate(boot = ifelse(GRanges(.) %over% osa.boot, T, F)) %>%
  dplyr::group_by(boot) %>%
  dplyr::tally() %>%
  dplyr::mutate(frac = n/sum(n),
                id = 'Boot')
ol.dep
ol.dep.n <- ol.dep[ol.dep$osa,]$n
ol.boot.n <- ol.shuffle[ol.boot$boot,]$n
dep.sum <- sum(ol.dep$n)
boot.sum <- sum(ol.boot$n)
ol.boot.n

prop.test(c(ol.dep.n, ol.boot.n), c(dep.sum, boot.sum), correct = T)
```

#### diffbind groups heatmap
```{r}
...
#### diffbind dynamic
```{r}
dds.peaks.qF <- dds.peaks.qF %>%
  data.frame() %>%
  dplyr::mutate(faire.3LW.zScore = get_bw_score(faire.wt.ssPool[faire.wt.ssPool$baseName == 'wt_3LW',]$bigwig_rpgcNorm_zNorm[1], GRanges(.)),
...
set.seed(11)
km <- kmeans(dds.mat, 2, nstart = 24, iter.max = 15)
...
dds.df <- dplyr::left_join(dds.dep.df, k_df)
...
#  dplyr::ungroup() %>%
#  dplyr::mutate(frac = n / sum(n)) %>%
#  ggplot(aes('', frac, fill = k)) + 
#  geom_bar(stat = 'identity', position = 'stack')  +
#  scale_fill_manual(values = c('#d4536a','grey60')) +
#  geom_text(aes(label = paste0('n=',n,' (',round(frac*100, digits = 2),'%)'), y = frac), vjust = 2, size = 6) +
#  theme(axis.title = element_blank(),
#        axis.text = element_text(size = 12),
#        axis.ticks.x = element_blank())
#ggsave('rPlots/4-draft-osaDep-temporal.png', width = 3, height = 6 , dpi = 300)
...
fairePeaks.deg %>%
  data.frame() %>%
  ggplot(aes(log2FoldChange, stat)) +
  geom_point() 
...
fairePeaks.deg %>%
  data.frame() %>%
  dplyr::mutate(logcat = dplyr::case_when(log2FoldChange <= 0.5 & log2FoldChange >= -0.5 ~ '0.5',
                                         log2FoldChange <= 1 & log2FoldChange >= -1  ~ '1',
                                         log2FoldChange > 1 ~ 'osa dependent', 
                                         log2FoldChange < -1 ~ 'osa ectopic',
                                         T ~ 'other')) %>%
  dplyr::group_by(logcat) %>%
  dplyr::tally() %>%
  dplyr::mutate(frac = n/sum(n))
```
